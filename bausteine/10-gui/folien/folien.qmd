...
## Zusammengesetzte Controls 1/2 {.smaller}
...
{{< include projekt/13-good-to-know-custom-control/MainWindow.axaml >}}
```
:::
::: {.column width=4%}
:::
::: {.column width=26%}
[]{.up10}
![](bilder/13-good-to-know-custom-control.png){class="drop-shadow"}
:::
:::
...
- Idee: Mehrere Controls zu einem eigenen Control zusammenfassen
    - `RectangularSectionControl` bündelt Beschriftungen und Textfelder
- Zeile 4: Namensraum des Projekts unter dem Kürzel `local` verfügbar machen
- Zeilen 10 und 13: Eigenes Control wie jedes andere Control verwenden

## Zusammengesetzte Controls 2/2 {.smaller}
...
{{< include projekt/13-good-to-know-custom-control/RectangularSectionControl.axaml >}}
```

[]{.down20}

- Aufbau wie bei einem Fenster
    - Wurzelelement ist `UserControl` statt `Window`
- Inhalt wie gewohnt: `Grid` mit Beschriftungen und Textfeldern
:::
::: {.column width=45%}
```{.csharp filename="RectangularSectionControl.axaml.cs"}
{{< include projekt/13-good-to-know-custom-control/RectangularSectionControl.axaml.cs >}}
```

[]{.down20}

- Code-Behind wie beim Fenster
- Eigene Methoden ergänzen
    - `CreateSection` liest die Eingaben aus und erzeugt ein `RectangularSection`-Objekt
- Aufruf im Fenster: `this.Section1Ctrl.CreateSection()`
:::
:::
:::
