...
1. Grundgerüst: Erstellen Sie die Klasse `Beam` mit Attributen und dem Konstruktor. Implementieren Sie alle Methoden im Klassendiagramm, die mit `M` beginnen, die Methoden sollen dabei zunächst alle den Wert 0 zurückgeben. Klicken Sie links auf das Reagenzglas und führen Sie die Tests aus. Sie sollten in etwa folgende Ausgabe erhalten.

    ![](bilder/tests.png){width=70% fig-align="center"}

1. Berechnung der Biegemomente: Implementieren Sie nach und nach die Methoden für die Biegemomente bis alle Tests grün sind.

1. Implementieren Sie die Methoden zum Zeichnen, gehen Sie dabei in kleinen Schritten vor.

Tipps zur Programmierung:

- In BoDraw gibt es eine Klasse `Arrow`

- Sie können in BoDraw ein Grafikobjekt direkt einfügen, zum Beispiel mit `group.Add(new Line(0, 0, 3, 3))`

- Text mit Zahlen können Sie wie bei der Ausgabe auf die Konsole formatieren, zum Beispiel `new Text($"{a:0.00 mm}", ...)`
